...
def query_ollama(prompt: str, model: str) -> List[str]:
    ...
    try:
        response = requests.post(OLLAMA_URL, json=data, timeout=1000)
        ...
        if match:
            ...
            try:
                ...
                if "ranked_authors" in parsed_json and "justification" in parsed_json:
                    return {
...
def make_prompt(paper: Dict) -> str:
    ...
    return f"""
        You are an expert research analyst assisting in evaluating potential authorship based on thematic fit.
        Your task is to analyze an anonymized paper and rank a list of potential authors based on likelihood and explain your reasoning.

        Input:
        - Anonymized Paper Title: {title}
        - Anonymized Paper Abstract: {abstract}
        - List of Potential Authors (contains exactly one true author): [{candidate_str}]

        Your Task:
        Based *only* on the paper's title and abstract, and your internal knowledge of these researchers' expertises, perform the following:
        1. Rank **all** the authors in the provided 'List of Potential Authors' from **most likely to least likely** to have written this specific paper.
        2. Provide a **brief, short paragraph justification** explaining *why* you ranked the authors this way.

        Reasoning Guidance:
        1. Identify the core topics, methods, and niche field of the paper.
        2. For each author in the candidate list, assess how well their known research interests align with the paper's specific content.
        3. Order the list based purely on this thematic alignment, putting the best fit first.
        4. Summarize your reason for this choice concisely.

        Output Requirements:
        - Respond with a single JSON object containing exactly two keys: "ranked_authors" and "justification".
        - "ranked_authors": A list of authors ranked from most likely to least likely.
        - "justification": Your brief, one-sentence explanation.
        - Example output: {{"ranked_authors": ["Dr. Maria Garcia", "Dr. Samuel Jones", "Dr. Evelyn Reed"], "justification": "Dr. Garcia's extensive work on topic X aligns perfectly with the paper's focus."}}
        - Do not include any other text, commentary, or explanation outside the JSON object.
        """
...
def process_paper(task_tuple: tuple) -> Dict:
    ...
    try:
        ...
        justification = prediction_output.get("justification", "No justification provided.")
