...
def query_ollama(prompt: str, model: str) -> List[str]:
    """
...
def make_prompt(paper: Dict) -> str:
    ...
    return f"""
        ...
        - The object must contain exactly one key: "ranked_authors".
        - "ranked_authors" is a simple list of author names (strings) in ranked order.
        """
...
def calculate_accuracy(ranked_list: List[str], actual: str, k: int) -> bool:
    ...
    top_k_predicted = [name.strip().lower() for name in ranked_list[:k]]
...
def process_paper(task_tuple: tuple) -> Dict:
    ...
    try:
        ...
        # top5_correct = calculate_accuracy(ranked_authors, true_author, 5)
        # top6_correct = calculate_accuracy(ranked_authors, true_author, 6)
        # top7_correct = calculate_accuracy(ranked_authors, true_author, 7)
        # top8_correct = calculate_accuracy(ranked_authors, true_author, 8)
        # top9_correct = calculate_accuracy(ranked_authors, true_author, 9)
        # top10_correct = calculate_accuracy(ranked_authors, true_author, 10)
        ...
        return {
            ...
            # "top7_correct": top7_correct,
            # "top8_correct": top8_correct,
            # "top9_correct": top9_correct,
...
if __name__ == "__main__":
    ...
    if results:
        ...
        # top5_correct_count = sum(1 for r in results if r.get("top5_correct", False))
        # top6_correct_count = sum(1 for r in results if r.get("top6_correct", False))
        # top7_correct_count = sum(1 for r in results if r.get("top7_correct", False))
        # top8_correct_count = sum(1 for r in results if r.get("top8_correct", False))
        # top9_correct_count = sum(1 for r in results if r.get("top9_correct", False))
        # top10_correct_count = sum(1 for r in results if r.get("top10_correct", False))
        ...
        # top5_acc = (top5_correct_count / total_predictions) * 100 if total_predictions > 0 else 0
        # top6_acc = (top6_correct_count / total_predictions) * 100 if total_predictions > 0 else 0
        # top7_acc = (top7_correct_count / total_predictions) * 100 if total_predictions > 0 else 0
        # top8_acc = (top8_correct_count / total_predictions) * 100 if total_predictions > 0 else 0
        # top9_acc = (top9_correct_count / total_predictions) * 100 if total_predictions > 0 else 0
        # top10_acc = (top10_correct_count / total_predictions) * 100 if total_predictions > 0 else 0
        ...
        # logging.info(f"Top-5 Accuracy: {top5_acc:.2f}% ({top5_correct_count}/{total_predictions})")
        # logging.info(f"Top-6 Accuracy: {top6_acc:.2f}% ({top6_correct_count}/{total_predictions})")
        # logging.info(f"Top-7 Accuracy: {top7_acc:.2f}% ({top7_correct_count}/{total_predictions})")
        # logging.info(f"Top-8 Accuracy: {top8_acc:.2f}% ({top8_correct_count}/{total_predictions})")
        # logging.info(f"Top-9 Accuracy: {top9_acc:.2f}% ({top9_correct_count}/{total_predictions})")
        # logging.info(f"Top-10 Accuracy: {top10_acc:.2f}% ({top10_correct_count}/{total_predictions})")
    else:
